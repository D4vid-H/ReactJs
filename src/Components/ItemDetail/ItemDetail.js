...
function ItemDetail({ id, nombre, picturUrl, informacion, categoriName, descripcion, precio, stock }) {
  ...
  const agregarProd = (stock, count) => {
    ...
    (stock - count >= 0 &&
      count > 0) && addItem({...prodObj, quantity: count})
  };
        ...
        <div className="md:flex items-center -mx-10">
            <div className="w-full md:w-1/2 px-10 mb-10 md:mb-0">
                <div className="relative border-4 border-yellow-200 p-4">
                    {picturUrl ? (
                      <img src={productImg(picturUrl)} alt="nada" className="w-full relative z-10 " />
                    ) : (
                      "Cargando..."
                    )}                
                </div>
            ...
            <div className="w-full md:w-1/2 px-10">
                <div className="mb-10">
                    ...
                    <h2 className="font-medium capitalize text-lg mb-2">{descripcion}</h2>
                ...
                <div>
                    ...
                    { isInCart({id}) ? <Link to='/cart' className="bg-blue-700 opacity-75 hover:opacity-100 text-green-100 hover:text-gray-900 rounded-full px-10 py-2 font-semibold">Ir al carrito</Link> : <ItemCount stock={9} inicio={1} onAdd={agregarProd} />}    
                    </div>
                </div>
            </div>
        </div>
