      ...
      getDocs(query(collectionsRef, where (documentId(), 'in', ids)))
            .then(response => {
              response.docs.forEach( doc => {
                  ...
                  const prodQuantity = getCart().find(prod => prod.id === doc.id)?.quantity;
                  ...
                  console.log(error);
                  setNotification("error", `No se pudieron agregar por lata de stock: ${error}`);
          }).finally(() => {
                setShowModal(false);            
                clear();
                navigate('/');
              ...
              <div className="border-0 rounded-lg shadow-lg relative flex flex-col w-full bg-white outline-none focus:outline-none">
                ...
                <div className="relative p-6 flex-auto">
                  <form className="bg-gray-200 shadow-md rounded px-8 pt-6 pb-8 w-full">
                    ...
                    <input onChange={(evt) => setSurName(evt.target.value)}
                    className="shadow appearance-none border rounded w-full py-2 px-1 text-black" />
                    <label className="block text-black text-sm font-bold mb-1">
                      Email
                    </label>
